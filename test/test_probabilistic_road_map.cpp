...
int main(int argc, char* argv[])
{
  ...
  starts.emplace_back(start_time + std::chrono::seconds(
    ...
    plan.initial_orientation * M_PI / 180.0);
  ...
  for (auto lane : graph.lanes_from(0))
  {
    std::cout << graph.get_lane(lane).entry().waypoint_index() << " " <<
      graph.get_lane(lane).exit().waypoint_index() << std::endl;
  }
  std::cout << "===========\n";
  for (auto lane : graph.lanes_from(1))
  {
    std::cout << graph.get_lane(lane).entry().waypoint_index() << " " <<
      graph.get_lane(lane).exit().waypoint_index() << std::endl;
  }
  ...
  rmf_traffic::agv::Planner::Debug planner_debug(planner_1);
  rmf_traffic::agv::Planner::Debug::Progress progress =
    planner_debug.begin(starts, goal, planner_1.get_default_options());
  ...
  while (app_window.isOpen())
  {
    ...
    rmf_planner_viz::draw::do_planner_debug(
      ...
      planner_1, starts, goal,
      graph.num_waypoints(), planner_debug, progress, start_time,
