...
int main(int argc, char* argv[])
{
  ...
  const std::string test_map_name = "L1";
  ...
  {
    graph_0 = rmf_fleet_adapter::agv::parse_graph(argv[1], traits);
  }
...
//        database);
  ...
  {
    const std::size_t start_idx = graph_0.find_waypoint("start")->index();
    const Eigen::Vector2d p0 = graph_0.get_waypoint(start_idx).get_location();
    const auto& lanes =  graph_0.lanes_from(start_idx);
    assert(!lanes.empty());
    const auto& l = graph_0.get_lane(lanes.front());
    const Eigen::Vector2d p1 = graph_0.get_waypoint(l.entry().waypoint_index()).get_location();
    const auto angle = std::atan2(p1[1]-p0[1], p1[0]-p0[0]);
    starts.emplace_back(now, start_idx, angle);

    goal = rmf_traffic::agv::Planner::Goal(graph_0.find_waypoint("goal")->index());
  }

  const Eigen::Vector2d p_goal = graph_0.get_waypoint(goal.waypoint()).get_location();
...
//    {graph_0_drawable.bounds(), graph_1_drawable.bounds()}, 0.02);
  rmf_planner_viz::draw::Fit fit({graph_0_drawable.bounds()});
  ...
  while (app_window.isOpen())
  {
    ...
    const bool take_snapshot = rmf_planner_viz::draw::do_planner_debug(
...
//    app_window.draw(graph_1_drawable, states);
    ...
    sf::CircleShape goal_circle;
    const double goal_R = 3.0;
    goal_circle.setRadius(goal_R);
    goal_circle.setPointCount(30);
    goal_circle.setFillColor(sf::Color::Magenta);
    goal_circle.setOrigin(goal_R, goal_R);
    goal_circle.setPosition(p_goal.x(), p_goal.y());
    goal_circle.scale(1.0, 1.0);
    goal_circle.rotate(90.0);
    goal_circle.setOutlineColor(sf::Color::Black);
    goal_circle.setOutlineThickness(goal_R/6.0);
    app_window.draw(goal_circle, states);

    if (take_snapshot)
    {
      sf::Texture texture;
      texture.create(app_window.getSize().x, app_window.getSize().y);
      texture.update(app_window);
      const std::string filename = "/home/grey/Desktop/planner/screenshot_" + std::to_string(screenshot_count++) + ".png";
      const auto image = texture.copyToImage();
      std::cout << "Image size: " << image.getSize().x << ", " << image.getSize().y << std::endl;
      if (image.getSize().x > 0 && image.getSize().y > 0)
      {
        const auto c = image.getPixel(0, 0);
        std::cout << "First pixel: " << (int)c.r << " " << (int)c.g << " " << (int)c.b << " "  << (int)c.a << std::endl;
      }
